...
`save` to write progress, or `quit` to stop.
...
`status` column derives it from `value`. Blanks become
`"not_reported"`, and `"NA"` or `"N/A"` are honoured as shorthand for
`"not_applicable"`.

## Build the report

`instar_report()` returns *data*, not a plot. It carries the paper
metadata, the resolved item table, and a coverage summary. Rendering is a separate
