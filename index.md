...
The unit of work is a single file, `INSTAR.csv`: one row per item, with
a `report` column you fill in. Write a sentence or two for each item
...
machine, without any of it being transmitted anywhere — and once the
page has loaded, it keeps working offline.
